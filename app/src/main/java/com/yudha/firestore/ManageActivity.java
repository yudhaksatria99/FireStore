...
public class ManageActivity extends BaseActivity implements View.OnClickListener  {
    ...
    private void addDocumentWithSpecificId(final String docId) {
        colRefUsers.document(docId).set(mFirebaseFcafas).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
    ...
    private void updateNestedObject(final String docId) {
        ...
        nestedData.put(FIELD_FIRST_NAME, "Firebase");
        nestedData.put(FIELD_LAST_NAME, "Fcfas");
    ...
    private void batchMultipleWrite() {
        ...
        DocumentReference docRef1 = colRefUsers.document(DOCUMENT_FIREBASE_FCFAS);
        mFirebaseFcafas.born = 2019;
        batch.set(docRef1, mFirebaseFcafas);

        DocumentReference docRef2 = colRefUsers.document("Firebaser");
        batch.update(docRef2, FIELD_DATE_TIME, FieldValue.serverTimestamp());

        DocumentReference docRef3 = colRefUsers.document("Firebase");
